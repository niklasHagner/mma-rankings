...
def past_events_to_file():
    ...
    # Save to file
    with open('../data/pastEventUrls.txt', 'w', encoding='utf-8') as file:
        for url in urls:
...
def future_events_to_file():
    ...
    # Extract URLs from good rows
    urls = [row.find_all("td")[0].find("a")['href'] for row in good_rows if row.find_all("td")[0].find("a")]

    # Save to file
    with open('../data/futureEventUrls.txt', 'w', encoding='utf-8') as file:
        for url in urls:
            file.write(url + '\n')  # Write each URL to the file, followed by a newline character
